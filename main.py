...
class MainApp(ctk.CTk):
    def __init__(self):
        ...
        # Загрузка фона
        bg = ctk.CTkImage(Image.open("res\\images\\zxc.png"), size=(700, 400))
        ...
        tables_menu.add_command(label="Муз инструменты", command=lambda: self.show_table('''
                    SELECT muz_instr.id_muz_instr,muz_instr.name , muz_instr.price
                    FROM muz_instr
                    JOIN sale ON muz_instr.id_vid = muz_instr.id_vid
        '''))
        tables_menu.add_command(label="Вид муз инструментов", command=lambda: self.show_table('''
                    SELECT * FROM vid_muz_instr
        '''))
    ...
    def show_table(self, sql_query):
        ...
        conn = sqlite3.connect("res\\book_bd.db")
        cursor = conn.cursor()
